...
const fileFilter = (req, file, cb) => {
    ...
    const allowedTypes = {
        audios: ['audio/mpeg', 'audio/wav', 'audio/aac', 'audio/ogg', 'audio/m4a', 'audio/webm'],
        word: ['application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-word.document.macroEnabled.12', 'application/vnd.ms-word.template.macroEnabled.12'],
...
const uploadFile = multer({
    storage: storage,
    limits: { 
        fileSize: 200 * 1024 * 1024, // 200MB por defecto
    ...
    try {
        ...
        if (missingFields.length > 0) {
            return res.status(400).json({
                ...
                detalle: 'Asegúrate de enviar: type, titulo, grado, subtipo'
            });
        }
            ...
            case 'PDF':
                if (!req.file && !isEdit) {
                    return res.status(400).json({
                        ...
                        detalle: `Debes proporcionar un archivo para ${req.body.type}`
                    });
                }
                ...
                if (req.file) {
                    ...
                    if (isEdit) {
                        ...
                        if (existingFile && existingFile !== metadata) {
                            normalizeAndDeleteOldFile(existingFile, baseDir);
                        }
                    }
                } else {
                    if (isEdit) {
                        ...
                        const existingMetadata = cleanMetadata(existingFile || req.body.existing_metadata || '');
            ...
            case 'Videos':
                ...
                if (isEdit) {
                    const currentIcon = await new Promise((resolve, reject) => {
                        req.db.query(
                            'SELECT MUL_IMAGEN FROM CAS_MULTIMEDIA WHERE MUL_ID = ?',
                            [multimediaId],
                            (error, results) => {
                                if (error) reject(error);
                                else {
        ...
        // Solo Videos requiere miniatura
        ...
        // Para otros tipos (Word, Excel, PDF, Audios), iconPath puede estar vacío
        if (req.body.type !== 'Videos') {
        ...
        const registro = {
            ...
            subtipo_id: parseInt(subtipo_id),
...
exports.handleDelete = async (req, res) => {
    try {
        ...
        const multimediaInfo = await new Promise((resolve, reject) => {
            req.db.query(
                'SELECT MUL_ENLACE, MUL_IMAGEN FROM CAS_MULTIMEDIA WHERE MUL_ID = ?',
                [multimediaId],
                (error, results) => {
                    if (error) reject(error);
        ...
        const deleteResult = await new Promise((resolve, reject) => {
            req.db.query(
                'DELETE FROM CAS_MULTIMEDIA WHERE MUL_ID = ?',
                [multimediaId],
                (error, results) => {
                    if (error) {
